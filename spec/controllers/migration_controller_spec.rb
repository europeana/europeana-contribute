  ...
  describe 'POST create' do
    context 'with valid params' do
      ...
      describe 'place annotations' do
        before do
          params[:contribution][:ore_aggregation_attributes][:edm_aggregatedCHO_attributes][:dcterms_spatial_places_attributes] = place_attributes
        end
        ...
        context 'with both places' do
          ...
          it 'saves them with skos:note' do
            post :create, params: params
            expect(assigns(:contribution).ore_aggregation.edm_aggregatedCHO.dcterms_spatial_places.size).to eq(2)
            expect(assigns(:contribution).ore_aggregation.edm_aggregatedCHO.dcterms_spatial_places.first.skos_note).to match(/began/)
            expect(assigns(:contribution).ore_aggregation.edm_aggregatedCHO.dcterms_spatial_places.last.skos_note).to match(/ended/)
          end
        end
        ...
        context 'with neither place' do
          ...
          it 'does not save them' do
            post :create, params: params
            expect(assigns(:contribution).ore_aggregation.edm_aggregatedCHO.dcterms_spatial_places.size).to be_zero
          end
        end
        ...
        context 'with only begin' do
          ...
          it 'saves it with skos:note' do
            post :create, params: params
            expect(assigns(:contribution).ore_aggregation.edm_aggregatedCHO.dcterms_spatial_places.size).to eq(1)
            expect(assigns(:contribution).ore_aggregation.edm_aggregatedCHO.dcterms_spatial_places.first.skos_note).to match(/began/)
          end
        end
        ...
        context 'with only end' do
          ...
          it 'saves it with skos:note' do
            post :create, params: params
            expect(assigns(:contribution).ore_aggregation.edm_aggregatedCHO.dcterms_spatial_places.size).to eq(1)
            expect(assigns(:contribution).ore_aggregation.edm_aggregatedCHO.dcterms_spatial_places.first.skos_note).to match(/ended/)
          end
        end
      end
